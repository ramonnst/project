        ...
        <nav class="page-side-nav" role="navigation">
            <div id="mainmenu" class="panel" style="border:0px" >
                ...
                <?php foreach ($menuSystem as $topMenuItem) { ?>
                    <a href="#<?= $topMenuItem->Id ?>" class="list-group-item <?php if ($topMenuItem->Selected) { ?>  active-menu-title <?php } ?>" data-toggle="collapse" data-parent="#mainmenu"><span class="<?= $topMenuItem->CssClass ?> __iconspacer"></span><?= $lang->_($topMenuItem->VisibleName) ?> </a>
                    <div class="collapse  <?php if ($topMenuItem->Selected) { ?> active-menu in <?php } ?>" id="<?= $topMenuItem->Id ?>">
                        <?php foreach ($topMenuItem->Children as $subMenuItem) { ?>
                            <?php if ($subMenuItem->Url == '') { ?>
                                ...
                                <a href="#<?= $topMenuItem->Id ?>_<?= $subMenuItem->Id ?>" class="list-group-item <?php if ($subMenuItem->Selected) { ?>  active-menu-title <?php } ?>" data-toggle="collapse" data-parent="#<?= $topMenuItem->Id ?>">
                                ...
                                <div class="collapse <?php if ($subMenuItem->Selected) { ?> active-menu in <?php } ?>" id="<?= $topMenuItem->Id ?>_<?= $subMenuItem->Id ?>" >
                                    <?php foreach ($subMenuItem->Children as $subsubMenuItem) { ?>
                                        <?php if ($subsubMenuItem->IsExternal == 'Y') { ?>
                                            <a href="<?= $subsubMenuItem->Url ?>" target="_blank" class="list-group-item menu-level-3-item <?php if ($subsubMenuItem->Selected) { ?> active <?php } ?>"><?= $lang->_($subsubMenuItem->VisibleName) ?></a>
                        ...
                        <?php } elseif ($subMenuItem->IsExternal == 'Y') { ?>
                                <a href="<?= $subMenuItem->Url ?>" target="_blank" class="list-group-item <?php if ($subMenuItem->Selected) { ?> active <?php } ?>"  aria-expanded="<?php if ($subMenuItem->Selected) { ?>true<?php } else { ?>false<?php } ?>">
                                  <div style="display: table;width: 100%;">
                                    <div style="display: table-row">
                                      <div style="display: table-cell"><?= $lang->_($subMenuItem->VisibleName) ?></div>
                                        <div style="display: table-cell; text-align:right; vertical-align:middle;"><span class="<?= $subMenuItem->CssClass ?>"></span></div>
